...
def _llm_available() -> bool:
    return True  # luôn có LLM cloud (gemini|openai); lỗi tạm -> caller rơi về extractive
...
def chat_turn_stream(store, session_id: str, question: str):
    ...
    # ----- smalltalk (chào / cảm ơn / ok / ừ / không / ?): đáp mẫu, KHÔNG đụng RAG -----
    from rag.advanced.smalltalk import detect as _detect_smalltalk, REPLIES

    intent = _detect_smalltalk(question)
    if intent:
        text = REPLIES[intent]
        result = {"answer": text, "sources": [], "grounded": True,
                  "mode": f"smalltalk:{intent}"}
        yield ("delta", text)
        record("total_ms", (_time.perf_counter() - t0) * 1000)
        log_query(question, question, [], result, session_id=session_id, top_score=0.0)
        yield ("result", _finish(store, session_id, sess, question, question, result,
                                 smalltalk=True))
        return
    ...
    if key:
        ...
        if hit is not None:
            ...
            result = dict(result)
            result["cached"] = True
            result["top_score"] = round(top, 4)
            result["score_min"] = config.SCORE_MIN
    ...
    # điểm RRF nguồn tốt nhất + ngưỡng, để UI hiển thị theo dõi (không lưu vào lịch sử)
    result["top_score"] = round(top_score, 4)
    result["score_min"] = config.SCORE_MIN
    yield ("result", _finish(store, session_id, sess, question, standalone, result))


def _finish(store, session_id, sess, question, standalone, result, smalltalk=False) -> dict:
    ...
    # Không lấy "chào"/"cảm ơn" làm tiêu đề — đợi câu hỏi thật đầu tiên.
    if not sess.get("title") and not smalltalk:
